...
int main(void) {
    ...
    enablePin(GPIOA, 0, GPIO_ANALOG);

    // Enable PA8 as a GPIO input pin for the capsensor
    enablePin(GPIOA, 8, GPIO_INPUT);

    // Enable PA6 for a test LED
    enablePin(GPIOA, 6, GPIO_OUTPUT);  

    // Enable PA5 as the output of capsense to the FPGA
    enablePin(GPIOA, 5, GPIO_OUTPUT);

    // Enable PB3 as the output of ir to the FPGA
    enablePin(GPIOB, 19, GPIO_OUTPUT);

    // Enable PA12 as an E-Stop input pin
    enablePin(GPIOA, 12, GPIO_INPUT);
